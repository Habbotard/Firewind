...
using System.Linq;
using Firewind.HabboHotel.Achievements.Composer;
...
namespace Firewind.HabboHotel.Achievements
{
    ...
    //        {
    //            Achievements.Add((uint)Row["id"], new Achievement((uint)Row["id"], (int)Row["levels"], (string)Row["badge"], (int)Row["pixels_base"], (double)Row["pixels_multiplier"], FirewindEnvironment.EnumToBool(Row["dynamic_badgelevel"].ToString())));
    //        }
    //    }
    ...
    //        Session.GetMessageHandler().GetResponse().AppendString(FormatBadgeCode(Achievement.BadgeCode, Level, Achievement.DynamicBadgeLevel));

    //        if (Level > 1)
    //        {
    //            Session.GetMessageHandler().GetResponse().AppendString(FormatBadgeCode(Achievement.BadgeCode, (Level - 1), Achievement.DynamicBadgeLevel));
    //        }
    //        else
    //        {
