...
namespace Firewind.HabboHotel.Pathfinding
{
    ...
    struct SquarePoint
    {
        ...
        #region Constructor
        internal SquarePoint(int pX, int pY, int pTargetX, int pTargetY, byte SquareData, bool pOverride)
        {
            mX = pX;
            mY = pY;
        ...
        internal bool CanWalk
        {
            get
            {
                if (!mLastStep)
                {
                    if (!mOverride) return (mSquareData == 1 || mSquareData == 4);
                    else return true;
                }
                else
                {
