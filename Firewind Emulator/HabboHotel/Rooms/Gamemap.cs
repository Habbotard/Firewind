        ...
        private bool ConstructMapForItem(RoomItem Item, Point Coord)
        {
            try
            {
                ...
                if (mItemHeightMap[Coord.X, Coord.Y] <= Item.TotalHeight)
                {
                    ...
                    //if (Item.GetBaseItem().InteractionType == Firewind.HabboHotel.Items.InteractionType.pool)
                    //    mUserItemEffect[Coord.X, Coord.Y] = 1;
                    //else if (Item.GetBaseItem().InteractionType == Firewind.HabboHotel.Items.InteractionType.normslaskates)
                    //    mUserItemEffect[Coord.X, Coord.Y] = 2;
                    //else if (Item.GetBaseItem().InteractionType == Firewind.HabboHotel.Items.InteractionType.iceskates)
                    //    mUserItemEffect[Coord.X, Coord.Y] = 3;
                    //else if (Item.GetBaseItem().InteractionType == Firewind.HabboHotel.Items.InteractionType.lowpool)
                    //    mUserItemEffect[Coord.X, Coord.Y] = 4;
                ...
                // Set bad maps
                if (Item.GetBaseItem().InteractionType == Firewind.HabboHotel.Items.InteractionType.bed)
                    mGameMap[Coord.X, Coord.Y] = 3;
            }
            catch (Exception e)
            {
                Logging.LogException("Error during map generation for room " + room.RoomId + ". Exception: " + e.ToString());
                //Logging.WriteLine("Mapsize X: " + mItemHeightMap.GetUpperBound(0) + "Mapsize Y: " + mItemHeightMap.GetUpperBound(1));
                //Logging.WriteLine("Original mapsize: " + Model.MapSizeX + "Mapsize Y: " + Model.MapSizeY);
