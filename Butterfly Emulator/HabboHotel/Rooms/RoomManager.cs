...
namespace Butterfly.HabboHotel.Rooms
{
    class RoomManager
    {
            ...
            */
            ...
            foreach (DataRow Row in Data.Rows)
            {
                ...
                roomModels.Add(Modelname, new RoomModel((int)Row["door_x"],
                    ...
                    staticFurniture, ButterflyEnvironment.EnumToBool(Row["club_only"].ToString()), (string)Row["poolmap"]));
            }
        }
        #endregion
        ...
        {
                while (ButterflyEnvironment.GetGame().gameLoopActive_EXT)
                {
                    try
                    {
                        if (ButterflyEnvironment.GetGame().gameLoopEnabled_EXT == true)
                        {
                            ButterflyEnvironment.GetGame().RoomManagerCycle_ended = false;
                            ...
                            ButterflyEnvironment.GetGame().RoomManagerCycle_ended = true;
                        }
                        Thread.Sleep(ButterflyEnvironment.GetGame().gameLoopSleepTime_EXT);
                    }
        ...
        internal void OnCycle()
        {
            ...
            catch (Exception e) { Logging.LogThreadException(e.ToString(), "RoomManager.OnCycle Exception --> Not inclusive"); }
        }
        ...
        private bool WorkActiveRoomsRemoveQueue()
        {
            if (activeRoomsRemoveQueue.Count > 0)
            {
                lock (activeRoomsRemoveQueue.SyncRoot)
                {
                    ...
                    {
                        //Logging.WriteLine("remove active room!");
        ...
        internal void RoomCycleTask()
        {
            ...
            if (sinceLastTime.TotalMilliseconds >= 500 && roomCyclingEnabled)
            {
                ...
                foreach (Room Room in loadedRooms.Values)
                {
                    if (Room.mIsIdle)
                    {
                        //loadedRooms.Remove(Room.RoomId); <> Cagaría la cola esta línea de código ya que modifica la cola del foreach.
                        mToRemove.Add(Room);
                    }
                    else
                    {

                        if (Room.procesoTask == null || Room.procesoTask.IsCompleted == true)
                        {
                            Room.procesoTask = new System.Threading.Tasks.Task(Room.ProcessRoom);
                            Room.procesoTask.Start();
                        ...
                        else
                        {
                            ...
                            {
                                mToRemove.Add(Room);
                                Logging.WriteLine("Conflictive Room: " + Room.RoomId);
                            }
                        }
                    }
                }
        ...
        {
            //Logging.WriteLine("Removing from all rooms: " + data.Name + ";" + data.Category);
            lock (activeRoomsRemoveQueue.SyncRoot)
            {
        ...
        internal void RemoveAllRooms()
        {
            ...
            foreach (Room Room in loadedRooms.Values)
            {
                ...
                Console.Clear();
                Logging.WriteLine("<<- SERVER SHUTDOWN ->> ROOM ITEM SAVE: " + String.Format("{0:0.##}", ((double)i / length) * 100) + "%");
                i++;
            }
            Logging.WriteLine("Done disposing rooms!");
        }
