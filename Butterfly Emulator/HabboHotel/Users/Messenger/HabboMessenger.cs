...
namespace Butterfly.HabboHotel.Users.Messenger
{
    class HabboMessenger
    {
        ...
        internal void DeliverInstantMessage(string message, uint convoID)
        {
            ...
            InstantMessage.AppendString(ButterflyEnvironment.GetUnixTimestamp() + string.Empty);
...
8                =     Your friend is wanking*/
        ...
        internal ServerMessage SerializeRequests()
        {
            ...
            reply.AppendInt32((requests.Count > ButterflyEnvironment.friendRequestLimit) ? (int)ButterflyEnvironment.friendRequestLimit : requests.Count);
            reply.AppendInt32((requests.Count > ButterflyEnvironment.friendRequestLimit) ? (int)ButterflyEnvironment.friendRequestLimit : requests.Count);
            ...
            {
                i++;
                if (i > ButterflyEnvironment.friendRequestLimit)
                    break;
