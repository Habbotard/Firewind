    ...
    public class ModerationTool
    {
        ...
        internal void CloseTicket(GameClient Session, uint TicketId, int Result)
        {
            ...
            if (Client != null)
            {
                ...
                Client.GetMessageHandler().GetResponse().AppendString((ButterflyEnvironment.getHabboForId(Ticket.ModeratorId) != null) ? (ButterflyEnvironment.getHabboForId(Ticket.ModeratorId).Username) : "Undefined");
        ...
        {
            //ButterflyEnvironment.GetGame().GetClientManager().QueueBroadcaseMessage(Ticket.SerializeUpdate(), "fuse_mod");
        }
        ...
        internal static ServerMessage SerializeUserInfo(uint UserId)
        {
            using (IQueryAdapter dbClient = ButterflyEnvironment.GetDatabaseManager().getQueryreactor())
            {
                ...
                Message.AppendBoolean(ButterflyEnvironment.GetGame().GetClientManager().GetClientByUserID(Convert.ToUInt32(User["id"])) != null);
