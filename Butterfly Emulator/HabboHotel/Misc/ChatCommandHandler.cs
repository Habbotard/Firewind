...
namespace Butterfly.HabboHotel.Misc
{
    class ChatCommandHandler
    {
        ...
        internal void givescore()
        {
            ...
            {
                Habbo data = ButterflyEnvironment.getHabboForName(Params[1]);
                if (data != null)
                {
                    int Score = int.Parse(Params[2]);
                    Ranking.AddScoreToUserId(Score, (uint)data.Id, RankingType.COMPETITIONS);
        ...
        internal void pickall()
        {
            ...
            if (TargetRoom != null && TargetRoom.CheckRights(Session, true))
            {
                ...
                if (ButterflyEnvironment.InventoryLimit <= furniCount)
                {
        ...
        internal void globalpixels()
        {
            ...
            ButterflyEnvironment.GetGame().GetClientManager().QueuePixelsUpdate(amount);
        ...
        internal void refresh()
        {
            switch (Params[1])
            {
                ...
                case "navigator":
                    ButterflyEnvironment.GetGame().GetNavigator().Initialize(ButterflyEnvironment.GetDatabaseManager().getQueryreactor());
                    Session.SendNotif("Reloaded navigator.");
                    break;

                case "bans":
                    ButterflyEnvironment.GetGame().GetBanManager().LoadBans(ButterflyEnvironment.GetDatabaseManager().getQueryreactor());
